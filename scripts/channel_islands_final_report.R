...
df_1 <- read_csv("EAOP A-G Validation Evaluation_6_5_24.csv") %>%
  ...
  filter(q2_1 == "Channel Islands High School")
...
# Compute counts ensuring both gender and admission topic are included
counts <- df_1_q3_1 %>%
  drop_na(q2_4) %>%
...
# Filter data for Channel Islands High School and process it for plotting
df_1_q3_4 <- df_1 %>%
  drop_na(q3_4, q2_1) %>%
  filter(q2_1 == "Channel Islands High School") %>%
  count(q3_4, q2_4) %>%
...
df_1 %>%
  drop_na(q3_6, q2_1) %>%
  filter(q2_1 == "Channel Islands High School") %>% # Filtering for Channel Islands High School
  count(q3_6, q2_4) %>%
...
df_1_q3_8 <- df_1 %>%
  ...
  filter(q2_1 == "Channel Islands High School")  # Ensure this filter is correctly placed after `q2_1` is available
...
df_1 %>%
  drop_na(q4_2, q2_4) %>%
  filter(q2_1 == "Channel Islands High School") %>% # Filter for Channel Islands
  count(q4_2, q2_4) %>%
...
df_1_q4_4 <- df_1 %>%
  ...
  filter(q4_4 != "q4_4_5_text", q2_1 == "Channel Islands High School") %>%
...
df_1_q4_6 <- df_1 %>%
  ...
  filter(q2_1 == "Channel Islands High School", q4_6 != "q4_6_5_text") %>%
